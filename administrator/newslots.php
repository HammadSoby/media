...
{    
	...
	if ($upd ==1) 
	{
		...
		{
			$sSQL = "UPDATE  Slots Set SlotName ='" .  $txtName . "', SlotDescription = '" . $txtDescription . "', SlotCostPrice = " .  $txtSlotCostPrice . ", SlotFullSellingPrice = " .  $txtSlotFullSellingPrice . ", SlotSize_ID = " .$optionSize . ", SlotTime_ID = " .$optTimes . ", Live = " . $Live . "  Where Product_ID = " . $Product_ID . " And  SlotName = '" . $txtName . "' And  SlotDescription = '" . $txtDescription . "' And SlotCostPrice = " . $txtSlotCostPrice ; 
		}
		//print($sSQL);
		mysql_query($sSQL);
	...
	<br/>
	<? 
		$sSQL = "SELECT Slots.Product_ID, Slot_ID, SlotName, SlotDescription, SlotCostPrice, SlotRRPrice, SlotFullSellingPrice, SlotSize_ID, SlotTime_ID, SlotDeadline FROM Slots, Products Where Slots.Product_ID = Products.Product_ID And SlotName = '" . $Slt . "'  And ProductName = '" . $pro . "' And SlotDescription = '" . $Des . "' And SlotCostPrice = " . $Cst . " And Live = 0 Order By Slot_ID Limit 1" ;
		//print($sSQL);
		...
		while($thisAccs = mysql_fetch_array($tmpAccs, MYSQL_ASSOC))
		{
			...
			$SlotRRPrice = $thisAccs["SlotRRPrice"];
		}	
    ...
    <tr> 
      <td><font size="+1">Min Retail Price:</font></td>
    <td><input type="text" name="txtSlotRRPrice" value="<? echo($SlotRRPrice); ?>"/></td>
		</tr>
    ...
    <td><input type="text" name="txtSlotDeadline" value="<? echo($SlotDeadline); ?>"/></td>
	  	...
	  	<?
		$iloop = 1;
		$sSQL = "SELECT SlotTime_ID, SlotTime FROM SlotTimes Where Product_ID =" . $ProID ;
		//print($sSQL);
		$tmpAccs = mysql_query($sSQL);
		...
		$iloop = 1;
		$sSQL = "SELECT SlotSize_ID, SlotSize FROM  SlotSizes Where Product_ID =" . $ProID ;
		//print($sSQL);
		$tmpAccs = mysql_query($sSQL);
	...
	<table align="center" border="1">
	<tr>
	  <th bgcolor="#0000FF" colspan="8"><font size="+2"><font color="#FFFFFF">New Slots &nbsp; <a href="mainmenu.php" >back</a></font></font></th>
	</tr>
	<tr>
	 <th>Slot Name</th> <th>Slot Description</th> <th>Slot Cost Price</th> <th>Slot Details</th> <th>Provider Name</th>
	</tr>
	<? 
		$sSQL = "SELECT DISTINCT ProductName, SlotName, SlotDescription, SlotCostPrice, SlotSize_ID, SlotTime_ID FROM Slots, Products Where Slots.Product_ID = Products.Product_ID And Live = 0 Order By ProductName" ;
		//print($sSQL);
		$tmpAccs = mysql_query($sSQL);
		...
		while($thisAccs = mysql_fetch_array($tmpAccs, MYSQL_ASSOC))
		{
			...
			echo("<tr>");
			echo("<td align='center'> <a href = 'newslots.php?pro=" . $ProductName . "&Slt=" .  $SlotName ."&Des=" .  $SlotDescription ."&Cst=" .  $SlotCostPrice . "'>". $SlotName ." </a></td>" );
...
{
		$sSQL = "SELECT Product_ID FROM Products Where ProductName = '" . $sdName . "'" ;
		//print($sSQL);
		$tmpAccs = mysql_query($sSQL);
