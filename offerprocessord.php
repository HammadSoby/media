...
//$Ordername = (string)$num_cart_items   . " Item(s) in Order - Check Order Details" ;
	...
	// Add Transaction
		...
		$sSQL = $sSQL . " VALUES ('" . $sProductName . "','" . $payment_status . "'," . $payment_amount . ",'" . $payment_currency . "','" . $txn_id . "','" . $receiver_email . "','" . $payer_email . "','" . $first_name . "','" . $last_name . "','" . $address_street . "','" . $address_zip . "','" . $contact_phone . "','" . $payer_business_name . "','" . $payer_id . "','" . $address_city . "','" . $address_country . "' )";
		//	print($sSQL);
		mysql_query($sSQL);
	...
	//'payment_amount = 45
		...
		if ($memo <> "") {
			...
			$sSQL = "SELECT Slot_ID FROM SpecialOfferSlots WHERE SpecialOffers_ID =" .  $ProductID ;
			...
			while($thisAccs = mysql_fetch_array($tmpAccs, MYSQL_ASSOC))
			{
				...
				$sSQL = "INSERT INTO SlotOrdersItems (SlotOrders_ID, Slot_ID)" ;
				$sSQL = $sSQL . " VALUES (" . $Order_ID . "," . $Slot_ID . ")";
				mysql_query($sSQL);
		...
		//	$OrderedProvidersCompany[0] = $ProvidersCompany;
			...
			$to = "admin@mediaunited.co.uk"; // admin@theunited.media
