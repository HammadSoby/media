...
$(document).ready(function(){
  ...
  return $data;
         ...
         $get_data=$pdo->prepare("SELECT * FROM users WHERE username = ?");
  ...
  <?php
             if(isset($_GET['id'])){
               $id= $_GET['id'];
              if(!empty($id)){
               $get_order=$pdo->prepare("SELECT * FROM orders WHERE id= ?");
            ...
            <a id="print" onclick="window.print();" target="_blank" style="cursor:pointer" >Print Invoice</a>
            <?php foreach($details as $detail) {?>
        <ul>
            <li><span id="client_info_tit">Slot Id : </span><?php echo $detail['id']; ?></li>
            <?php
           
            $cemail=$pdo->prepare("SELECT email,fullname,adress,phone FROM users WHERE username = ? ");
            $cemail->BindValue(1,$detail['username']);
            $cemail->execute();
            $contactemail=$cemail->fetchAll();
            foreach($contactemail as $theemail);
            
            ?>
            <li><span id="client_info_tit">Purchaser Name : </span><?php echo $theemail['fullname']; ?></li>
            <li><span id="client_info_tit">Contact Email : </span><?php echo $theemail['email']; ?></li>
            <li><span id="client_info_tit">Telephone Number : </span><?php echo $theemail['phone']; ?></li>
            <li><span id="client_info_tit">Media Type  : </span><?php echo $detail['type']; ?></li>
            <li><span id="client_info_tit">Media Slot  : </span><?php echo $detail['slot']; ?></li>
            <li><span id="client_info_tit">Media Size  : </span><?php echo $detail['size']; ?></li>
            <li><span id="client_info_tit">Order Status : </span>
              <?php 
            $leorder_status=$detail['order_status'];
              if($leorder_status==0){
                echo "Incomplete","<small id='deta'>&nbsp; &nbsp; &nbsp; You have not made Payment for your Order.</small>";
              }
              if($leorder_status==1){
                echo "Paid & Pending","<small id='deta'> &nbsp; &nbsp; &nbsp; Your order is being processed and will show Accepted once fanilised</small>";
              }
              if($leorder_status==3){
                 echo "Accepted","<small id='deta'>&nbsp; &nbsp; &nbsp;  Your Order Has Been Validated. You'll Hear From Us In Your Email Soon</small>";
              }
            ?></li>
           
      
            <li><span id="client_info_tit">Production Broadcast Date(s) :</span> <?php echo $detail['dates'];?></li>
            <?php 
            ...
            ?>
            <li><span id="client_info_tit">Booked on :</span> <?php echo $detail['created'];?></li>
