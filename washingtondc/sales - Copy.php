...
<form target="paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr"
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		?>
		<form action="book.php?type=tv&rt=1" method="post">
		<input type="hidden" name="txtProvID" value="<? echo($Providers_ID); ?>" />
	   <table align="center">
	<tr>
	<td align="center">Select Slot</td>
	  <td width="500" >
        <select name="optSlot">
         
       
        <?
		$sSQL = "SELECT DISTINCT SlotName , SlotDescription ,  SlotFullSellingPrice FROM Slots Where Providers_ID = " . $Providers_ID . " Order by SlotName" ;
		//print($sSQL);
		$tmpAccs = mysql_query($sSQL);
		...
		while($thisAccs = mysql_fetch_array($tmpAccs, MYSQL_ASSOC))
		{
			...
			echo(" <option value='" . $SlotName . "'>" . $SlotName . "-" . $SlotDescription  . "- Price &pound;" . $SlotFullSellingPrice . "</option>");
		}
		?>
		 </select> 
		</td>
		<td height="400" width="200"> <input type="submit" id="booka" name="Submit2" value="Select"></td>
		</tr>
	...
	if ($rt ==1) 
	{
		$optSlot =  $_POST['optSlot'];
		$txtProvID =  $_POST['txtProvID'];
		 ?>
		 <table border="1" bordercolor="#0000FF" align="center">
		 <tr>
		 <th colspan="7">Purchase Slots</th>
		 </tr>
		 <tr>
		 <th>Slot</th>  <th>Description</th>  <th>Price</th>  <th>Quantity</th>  <th>Slot Details</th>  <th>Production Date</th> 
      <th align="center"  height='200' width='100'>Select <br/>All
        <input name="checkbox" type="checkbox" value="All" checked></th>
		 </tr>
		 <?
		 $loop = 1;
		$sSQL = "SELECT Slot_ID, SlotName, SlotDescription,  SlotFullSellingPrice, SlotQuantity, SlotDeadline, SlotSize_ID, SlotTime_ID FROM Slots Where SlotName = '" . $optSlot . "' And Providers_ID = " . $txtProvID. " Order By SlotDeadline" ;
		//print($sSQL);
		$tmpAccs = mysql_query($sSQL);
		...
		while($thisAccs = mysql_fetch_array($tmpAccs, MYSQL_ASSOC))
		{
			...
			echo("<td align='center'>" . $ProductionDate . "</td>" );
			echo("<td align='center' height='200' width='100'> <input  name='chk" .  $loop  . "' type='checkbox' value=" . $Slot_ID . " ></td>" );
